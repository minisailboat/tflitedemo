...
//  private static final int NUM_DETECTIONS = 10;
  ...
  public static Detector create(
      ...
      final int MaxResultsNum)
      throws IOException {
    return new TFLiteObjectDetectionAPIModel(context, modelFilename,MaxResultsNum);
  }

  private TFLiteObjectDetectionAPIModel(Context context, String modelFilename,int MaxResultsNum) throws IOException {
    modelBuffer = FileUtil.loadMappedFile(context, modelFilename);
//    optionsBuilder = ObjectDetectorOptions.builder().setMaxResults(NUM_DETECTIONS);//设置返回最大结果
    optionsBuilder = ObjectDetectorOptions.builder().setMaxResults(MaxResultsNum);//设置返回最大结果
